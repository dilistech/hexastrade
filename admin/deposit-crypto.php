    ...
    $sql = 'SELECT * FROM `account` INNER JOIN packages ON account.package_id = packages.id INNER JOIN user ON account.user_id = user.id WHERE packages.type = 1 && account.status = 0 ';
        ...
        $rows = $stmt->fetchAll();
         ...
         if (isset($_POST['deposit'])) {
            ...
            $r_update->execute(['interest_earned' => $amount * 0.25,'id' => $r_id]);
          ...
          $body .= '<h2>Your referred client made deposit .</h2>';
            ...
            $body .= '<p>Interest earned:'.$amount * 0.25.'</p>';   
        ...
        $body .= '<p style="margin:10px 0;">Congratulations your deposit is approved'.' '.'</p>';  
       $body .= '</div>'; 
        send_email($row -> email,'Investment Approved',$row -> first_name, $row -> last_name,$body,new PHPMailer(true));
